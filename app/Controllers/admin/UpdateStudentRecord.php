 ...
 */
class UpdateStudentRecord extends BaseController {
...
//session_start();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($vSname)) {
                 $k = 0;
                $l = 0;
                $data2 = array('Sname'=>$Sname,'Fname'=>$Fname,'mname'=>$mname,'regno' => $vRegno,'level'=>$level,'religion'=>$religion,'dob'=>$dob,'sex'=>$sex,'state'=>$state,'lga'=>$lga,'lastUpdated'=>$date,'LastUpdatedBy'=>$_SESSION['username']);
//var_dump($data2);die();
                $this->db->where(array('sn' => $sn));
                $this->db->update('students', $data2);
                if ($this->db->affected_rows() > 0) {
                    $k = $k + $this->db->affected_rows();
                    $name = $Sname." ".$Fname." ".$mname;
                    $this->db->set('RefNumb', $vRegno);
                    $this->db->set('candname', $name);
...
$_SESSION['msg'] = $msg;
                    ...
                    redirect('admin/UpdateStudentRecord', 'refresh');
    ...
    public function getSnamec() {
        ...
        $sql = "SELECT sn,regno,Sname,Fname,mname,level,state,lga,dob,sex,RegNumb FROM students WHERE RegNumb ='" . $q . "'";
