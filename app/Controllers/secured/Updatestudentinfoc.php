 ...
 */
class Updatestudentinfoc extends BaseController {

//put your code here
    public function index() {
//session_start();
        $data = array('title' => 'Student Profile update'
        );
        ...
        $this->form_validation->set_rules('nokemail', 'Nok Email Address', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data2 = array(
                'regno' => $this->session->userdata('RegNumb'),
                'Fname' => $this->session->userdata('lname'),
                ...
                'fact' => $this->session->userdata('FacAbrev'),
                'dept' => $this->session->userdata('CourseAbrev'),
                'degree' => $this->session->userdata('degree'),
                'nation' => $this->session->userdata('nation'),
                'state' => $this->session->userdata('StateOfOrigin'),
                'lga' => $this->input->post('LGA'),
                'mstatus' => $this->input->post('maritalstatus'),
                'religion' => $this->input->post('religion'),
                'ResidentialAddress' => $this->input->post('permaddr'),
                ...
                'admode' => $this->session->userdata('admode'),
                'sEmail' => $this->input->post('nokemail'),
                'hometown' => $this->input->post('hometown')
            );
//var_dump($data2);
            $this->regno = $this->session->userdata('RegNumb');
            $this->db->where('regno', $this->regno);
            $this->db->update('students', $data2);
            if ($this->db->affected_rows() > 0) {
                echo '<script>alert("Your profile updated was successful!!!")</script>';
                $this->session->set_flashdata('message', 'Your profile updated was successful!!!');
                redirect('secured/Updatestudentinfoc', 'refresh');
            } else {
                echo '<script>alert("Your profile updated was not successful!!!")</script>';
                $this->session->set_flashdata('message', 'Your profile update failed!!!');
            }
        }
    }
